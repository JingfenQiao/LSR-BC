...
#SBATCH --output=./log/mlm_open_pos%a.out
#SBATCH --error=./log/mlm_open_pos%a.out

epochs=(6 7 8 9)
epoch=${epochs[$SLURM_ARRAY_TASK_ID]}
path=/ivi/ilps/personal/jqiao/lsr-bc/query_adapter_model/mlm_all_docs_50negs_kl_mse
...
CUDA_VISIBLE_DEVICES=0 python -u bc/evaluate_beir.py \
  ...
  --benchmark BEIR \
  --batch_size 32 \
  --output_folder results/mlm_all_docs_50negs_kl_mse/epoch-$epoch
