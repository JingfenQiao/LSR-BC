...
def collate_batch(
    ...
    old_model_queries = [item[0] * 3 for item in items]
    new_model_queries = [item[0] for item in items]
    docs = [doc for item in items for doc in item[1]]
    group_size = len(items[0][1])

    q_old = tokenizer(
        old_model_queries, padding=True, truncation=True, max_length=max_q_len, return_tensors="pt", return_special_tokens_mask=True
    )
    q_new = tokenizer(
        new_model_queries, padding=True, truncation=True, max_length=max_q_len, return_tensors="pt", return_special_tokens_mask=True
    )
    d = tokenizer(
        docs, padding=True, truncation=True, max_length=max_d_len, return_tensors="pt", return_special_tokens_mask=True
    )
    ...
    return Batch(
        old_q_input_ids=q_old["input_ids"],
        old_q_attention_mask=q_old["attention_mask"],
        old_q_special_tokens_mask=q_old["special_tokens_mask"],
        new_q_input_ids=q_new["input_ids"],
        new_q_attention_mask=q_new["attention_mask"],
        new_q_special_tokens_mask=q_new["special_tokens_mask"],
        d_input_ids=d["input_ids"],
        d_attention_mask=d["attention_mask"],
        d_special_tokens_mask=d["special_tokens_mask"],
        group_size=group_size,
        ...
        with torch.no_grad():
            for batch in tqdm(val_dl, desc="Initial Validation"):
                ...
                with torch.cuda.amp.autocast(enabled=use_amp):
                    ...
                    q_vec = q_new(input_ids=batch.new_q_input_ids, attention_mask=batch.new_q_attention_mask,
                                  special_tokens_mask=batch.new_q_special_tokens_mask)
        ...
        for _ in tqdm(range(steps_per_epoch), desc=f"Epoch {ep+1}/{epochs}"):
            ...
            with torch.no_grad():
                with torch.cuda.amp.autocast(enabled=use_amp):
                    ...
                    # q_vec_old = doc_old(input_ids=batch.q_input_ids, attention_mask=batch.q_attention_mask,
                    #                 special_tokens_mask=batch.q_special_tokens_mask)
                    
                    q_vec_old = doc_old(input_ids=batch.old_q_input_ids, attention_mask=batch.old_q_attention_mask,
                                    special_tokens_mask=batch.old_q_special_tokens_mask)

            with torch.cuda.amp.autocast(enabled=use_amp):
                # q_vec_new = q_new(input_ids=batch.q_input_ids, attention_mask=batch.q_attention_mask,
                #               special_tokens_mask=batch.q_special_tokens_mask)
                q_vec_new = q_new(input_ids=batch.new_q_input_ids, attention_mask=batch.new_q_attention_mask,
                              special_tokens_mask=batch.new_q_special_tokens_mask)
                ...
                teacher_scores = torch.einsum("bv,bgv->bg", q_vec_old, d_vec) / max(temperature, 1e-6)
