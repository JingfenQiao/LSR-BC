...
def precompute(
    *,
    ...
    os.makedirs(os.path.join(out_dir, new_model_name), exist_ok=True)
    os.makedirs(os.path.join(out_dir, old_model_name), exist_ok=True)

    old = SparseEncoder(old_model_name).to(device)
    ...
    shard_idx = 0
    d_old_dict = {}
    main_file_path = os.path.join(out_dir, f"{new_model_name}_precompute.jsonl.gz")

    with gzip.open(main_file_path, 'wt') as out_f:
        with torch.no_grad():
            for batch in tqdm(train_dl, desc="Saving shards"):
                ...
                for b in range(B):
                    for g in range(G):
                        ...
                        if d_id not in d_old_dict:
                            d_old_dict[d_id] = d_old[idx].cpu()

                        if len(d_old_dict) == SHARD_SIZE:
                            torch.save(d_old_dict, os.path.join(out_dir, f'{old_model_name}/d_old_vecs_{shard_idx:06d}.pt'))
                            print(f"Saved {old_model_name} d_old_vecs shard {shard_idx}")
                            d_old_dict = {}
                            shard_idx += 1

    if d_old_dict:
        torch.save(d_old_dict, os.path.join(out_dir, f'{old_model_name}/d_old_vecs_{shard_idx:06d}.pt'))
        print(f"Saved final {old_model_name} d_old_vecs shard {shard_idx}")
...
def main():
    ...
    parser.add_argument("--out_dir", default="/fnwi_fs/ivi/irlab/projects/mllm_yk/jf/lsrbc/train_set/query_adapter/", type=str)
    ...
    parser.add_argument("--num_negs", default=100, type=int, help="More negatives = better, 7-15 common")
