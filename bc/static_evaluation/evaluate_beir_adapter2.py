...
from adapter.transformation_multitask import Old2NewAdapter
...
def filter_token_ids(sentences, reps, doc_encoder):
    ...
    for i, text in enumerate(sentences):
        ...
        term_ids_list.append(torch.tensor(keep_ids, dtype=torch.long, device=doc_encoder.device))
        weights_list.append(torch.tensor(keep_vals, dtype=torch.float, device=doc_encoder.device))

    return term_ids_list, weights_list



def avg_aggregate(batch_original_reps: torch.Tensor,
                    batch_adapter_reps: torch.Tensor) -> torch.Tensor:

    assert batch_original_reps.shape == batch_adapter_reps.shape

    # Boolean masks
    orig_nz = batch_original_reps != 0
    adap_nz = batch_adapter_reps != 0

    both = orig_nz & adap_nz
    only_orig = orig_nz & ~adap_nz
    only_adap = adap_nz & ~orig_nz

    out = torch.zeros_like(batch_adapter_reps)

    # keep the non-zero from whichever has it
    out[only_orig] = batch_original_reps[only_orig]
    out[only_adap] = batch_adapter_reps[only_adap]

    # average where both are non-zero
    out[both] = 0.5 * (batch_original_reps[both] + batch_adapter_reps[both])
        ...
        with torch.no_grad():
            if prompt_type == "query":
                ...
                for start in range(0, len(sentences), batch_size or len(sentences)):
                    ...
                    batch_original_reps = reps[start:end]
                    
                    batch_term_ids = old_term_ids[start:end]
                    batch_weights = old_weights[start:end]
                    batch_reps, _, _ = self.adapter(batch_term_ids, batch_weights)
                    # aggregate batch_reps and batch_original_reps if they share some common terms
                    # batch_merged = avg_aggregate(batch_original_reps, batch_reps)
                    batch_merged = 0.5 * batch_original_reps + 0.5 * batch_reps
                    
                    final_reps.append(batch_merged)
...
def main():
    ...
    adapter = Old2NewAdapter.from_pretrained("/ivi/ilps/personal/jkang1/jf/lsr-bc/adapter_checkpoints/old_sp_tiny_new_sp_v3_1000000_data_multitask_under_kl_dp_sota").to(torch.device(args.device))
    adapter.eval()
