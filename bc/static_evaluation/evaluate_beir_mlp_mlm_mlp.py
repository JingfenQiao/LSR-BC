...
from query_adapter.modelling_mlp_mlm import MultiHeadSparseEncoder
import argparse
import torch
import numpy as np
import json
...
class SparseEvaluator:
    def __init__(self, query_encoder=None, doc_encoder=None):
        self.query_encoder = MultiHeadSparseEncoder.from_pretrained(query_encoder)
    ...
    def encode(
        ...
        **kwargs,
        ...
        if prompt_type == "query":
            input_ids, attention_mask, special_tokens_mask = self.query_encoder.tokenize(sentences)
            sparse_mlm, sparse_mlp = self.query_encoder(input_ids, attention_mask, special_tokens_mask)
            # reps = sparse_mlm.to_sparse()
            reps = sparse_mlp.to_sparse()
        else:
