...
class SparseEvaluator:
    def __init__(self, old_model="naver/splade-v3", new_model="naver/splade-v3", fusion_type=1, device="cpu"):
    ...
    def fusion1(old_reps_decoded, new_reps_decoded):
        """
        Fusion strategy 1: Using weighting from new model but terms from old model.
        """
    ...
    def fusion2(old_reps_decoded, new_reps_decoded):
        """
        Fusion strategy 2: Averaging weights for all terms present in either model (union of terms).
    ...
    def fusion3(old_reps_decoded, new_reps_decoded):
        """
        fusion strategy 3: Max fusion (strongest signal): For each term: Take the maximum weight from either model (0 if absent).
        """
    ...
    def fusion4(old_reps_decoded, new_reps_decoded):
        """
        fusion strategy 4: Intersection with combined weights (High Precision): Only keep terms that appear in both representations, combining their weights (e.g., averaging or summing).
        """
    ...
    def encode(
        ...
        **kwargs,
        ...
        if prompt_type == "query":
            ...
            # old_reps_decoded, new_reps_decoded
...
def main():
    ...
    parser.add_argument("--fusion_type", type=int, default=1, choices=[1,2,3,4], help="Fusion strategy type")
    ...
    model_meta = ModelMeta(
        ...
        name=f"your_wandb_entity/old_{qe_safe}_new_{de_safe}_fusion{args.fusion_type}",
        languages=["eng-Latn"],
        modalities=["text"],
        revision="v1",
