        ...
        **kwargs,
        ...
        with torch.no_grad():
            if prompt_type == "query":
                ...
                decoded_reps = self.doc_encoder.decode(reps)
                old_term_ids, old_weights = convert_decoded_list(decoded_reps, self.doc_encoder.tokenizer)
                # Pad to same length
...
def main():
    ...
    adapter = Old2NewAdapter.from_pretrained("/ivi/ilps/personal/jkang1/jf/lsr-bc/adapter_checkpoints/old_sp_tiny_new_sp_v3_1000000_data_multitask_under").to(torch.device(args.device))
    adapter.eval()
